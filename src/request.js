...
export default class Request {
	...
	constructor(url, method, qs, body, headers, dataKey = 'data', errorKey = 'error', keepAlive = false, logWarning = false) {
	...
	_request() {
		return new Promise((resolve, reject) => {
			request[this._method]({
				...
				const { _meta: meta, warning } = body;
				if (warning && this._logWarning) {
					if (warning instanceof Array) {
						warning.forEach((warn) => console.warn(new Date(), warn));
					} else {
						console.warn(new Date(), warning);
					}
				}
